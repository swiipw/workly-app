...
import { 
    ...
    BookOpen, LogOut, Save, X, Plus, Check, Zap, ArrowLeft, Calendar, FileText, Upload, Loader2 // Añadido Loader2
} from 'lucide-react'; 
...
// Acepta `loggedInUser` (de App.jsx) para nombre/email y `onLogout` para cerrar sesión.
const ProfileScreen = ({ user: loggedInUser, onLogout }) => { // <--- CORRECCIÓN CLAVE: Aceptar props
    
    // Merge loggedInUser data with static INITIAL_USER_DATA for a complete profile object
    const initialUserData = {
        ...INITIAL_USER_DATA,
        name: loggedInUser?.name || INITIAL_USER_DATA.name,
        email: loggedInUser?.email || INITIAL_USER_DATA.email,
    };

    // Definimos el estado inicial usando los datos estáticos definidos arriba
    const [user, setUser] = useState(initialUserData);
    const [view, setView] = useState('profile');
    const [isLoggingOut, setIsLoggingOut] = useState(false); // Nuevo estado para el indicador de carga

    // FUNCIÓN DE CERRAR SESIÓN CORREGIDA
    const handleLogout = () => {
        setIsLoggingOut(true);
        // Simulación de latencia de cierre de sesión
        setTimeout(() => {
            console.log("Sesión cerrada. Llamando a onLogout()."); 
            setIsLoggingOut(false);
            if (onLogout) {
                onLogout(); // Llama a la función de App.jsx para cambiar el estado global
            }
        }, 800);
    };
            ...
            <div className="p-4 space-y-6">
                ...
                <div className="pt-4 pb-8">
                    <button
                        ...
                        disabled={isLoggingOut} // Deshabilita si está cargando
                        className="w-full py-3 flex items-center justify-center bg-red-600/90 text-white font-bold rounded-xl shadow-lg transition transform hover:scale-[1.01] disabled:bg-red-400 disabled:cursor-not-allowed"
                    >
                        {isLoggingOut ? (
